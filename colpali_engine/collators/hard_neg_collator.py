...
class TAIHardNegCollator(VisualRetrieverCollator):
    def __init__(
        self,
        processor: BaseVisualRetrieverProcessor,
        max_length: int = 2048,
    ...
    def __call__(self, examples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for example in tmp_examples:
            pos_image = Image.open(BytesIO(base64.b64decode(example["gold_index"])))

            # pos_image = example["gold_index"]
            pos_query = example["query"]

            # Randomly sample a negative image amongst the top 10
            neg_image = Image.open(BytesIO(base64.b64decode(example["negs"][0])))
            # neg_image = example["negs"][0]

            examples += [
                {"image": pos_image, "query": pos_query, "neg_image": neg_image}
            ]
